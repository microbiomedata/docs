...
In most of our Python projects, we use [Poetry](https://python-poetry.org/) to manage dependencies. While pip-tools is still in use on some repos, we are moving away from it (to Poetry).

_Examples: nmdc-schema (Poetry)_
...
_Examples: nmdc-field-notes (npm), nmdc-server (yarn)_
...
_Examples: nmdc-runtime, nmdc-schema_

### Docker

Projects which are designed to be run as deployed applications should be [Dockerized](https://www.docker.com/). This means that the project itself should define at least one [Dockerfile](https://docs.docker.com/get-started/docker-concepts/building-images/writing-a-dockerfile/). Once a Docker image is built according to the Dockerfile it can be pushed to the [GitHub Container Registry](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry). In a deployment environment, the appropriate image can be pulled from the container registry and a Docker container can be started using it.

Most Dockerized projects also use a [Docker Compose](https://docs.docker.com/compose/) file. This allows other containers necessary for local development (e.g. a database, a static file server, etc) to be run alongside the main application container.

_Examples: nmdc-server, nmdc-runtime_
...
_Examples: nmdc-schema, geoloc-tools_

### GitHub Actions

We use [GitHub Actions](https://docs.github.com/en/actions) for a variety of automation tasks. In general, we strive to automate as many tasks as possible. Ways we use GitHub Actions include:
...
_Examples: nmdc-field-notes, submission-schema, docs_

## Data pipeline/orchestration frameworks

There are a number of pipelines that produce data assets (ex. JSON, XML files, etc.) and accomplish various ad-hoc tasks (like asset ingest, scheduled routine jobs, etc.) in the NMDC runtime framework. It's important that we have a one stop-shop for the orchestration of these data pipelines, and we use [Dagster](https://docs.dagster.io/) as our solution of choice for this problem.

Repos: [nmdc-runtime](https://github.com/microbiomedata/nmdc-runtime)

## Bioinformatics workflows

NMDC workflows use public repositories written in Workflow Description Language (WDL) and public Docker containers for software package management
...
- Specify container versions in each WDL ideally at the task level
- Use docker in the `runtime` block and specify `time`, `cpu` and `memory`
  - If using a different container management tool like singularity and running outside of JAWS this specify that in the cromwell configuration file
- Adhere to other JAWS [recommended best practices](https://jaws-docs.readthedocs.io/en/latest/Resources/best_practices.html)
- Use SHA or tagged versions for docker containers
- Subworkflows should be imported using https imports from the github repo
- Use `meta` block to specify maintainer, version and email

### Validating WDLs

- Use womtool, miniwdl or jaws validate to confirm your WDL is valid: `miniwdl check --strict <wdl>`

### Docker containers

- Use organizational registries (eg. https://hub.docker.com/u/microbiomedata) not personal ones

### CI/CD

- Use github actions to validate WDLs ([example](https://github.com/microbiomedata/mg_annotation/blob/master/.github/workflows/wdl_linter.yml))
...
We use [Docker](https://www.docker.com/) and [docker-compose](https://docs.docker.com/compose/) to create local development environments. Specific setup instructions are included in the repo documentation

Repos: [nmdc-runtime](https://github.com/microbiomedata/nmdc-runtime), [nmdc-server](https://github.com/microbiomedata/nmdc-server)
...
It incorporates many elements from the GSC's MIxS standard, often imposing a higher level of structure. See [https://github.com/GenomicsStandardsConsortium/mixs](https://github.com/GenomicsStandardsConsortium/mixs)
...
- Follow standard LinkML naming conventions (UpperCamelCase for classes and enums, snake_case for slots). Element names should not include whitespace.
- Use LinkML linter to check style and conventions
- Class names should be nouns or noun-phrases (Person, SequenceAlignment, Annotation)
...
- Provide valid examples for all classes in `src/data/valid` and invalid/counter-examples for all classes in `src/data/invalid`
- Use enums for categorical values (finite sets)
- Reuse existing schema elements whenever possible. We do not need a `soil_temp` slot, and an `air_temp` slot, etc.
...
- Iterating over the `attributes` (not `slots`) in a `induced_class` is a very common pattern
...
- Use Python data classes from the `mdc-schema` Python package instead of generating dicts/JSON manually
...
NMDC uses [MongoDB](https://www.mongodb.com/) as the central metadata store for NMDC (meta)data, and is considered the system of record. The NMDC API / Runtime provides programmatic access to the MongoDB backend. NMDC API queries access data from MongoDB

### PostgreSQL

NMDC uses [PostgreSQL](https://www.postgresql.org/) for the NMDC [Data portal](https://data.microbiomedata.org/) and [Submission portal](https://data.microbiomedata.org/submission/home).

[SQLAlchemy](https://www.sqlalchemy.org/) is used to manage data migrations in postgres

The data portal DB is not a system of record for NMDC Data. It is a transformed copy of the system of record from MongoDB, transformed into a relational schema optimized for the sort of queries that the data portal client needs to make.

The Submission portal uses Postgres to store submission information, before it gets ingested into MongoDB.
...
## Use of LLMs across repositories

This is a new and fast evolving area. NMDC staff have experience using the following tools for code generation / acting as coding and writing assistants:  
[ChatGPT](http://chatgpt.com/), [Claude](https://claude.ai/), [Github Copilot](https://github.com/features/copilot), [Gemini](https://gemini.google.com/)

LBL users also have access to the [CBORG](https://cborg.lbl.gov/) system, which provides access to a number of commercial and internally hosted LLMs.

## Git and GitHub

All code being developed in the NMDC ecosystem/toolchain is open sourced and all repositories are made publicly accessible via GitHub, under the [microbiomedata](https://github.com/microbiomedata/) organization. By choosing GitHub as our code collaboration platform of choice, we are marrying ourselves to [git](https://git-scm.com/doc) as our de-facto version control tool. We follow the typical Git/GitHub workflow for getting “new code” (for bug fixes, feature enhancements, etc.) into our codebase which is: create an issue for your request → make a corresponding branch → add your commits to that branch → make a PR which will be reviewed by “owners” (experts) → merge into `main` → delete branch. Releases for the various repositories are also co-ordinated/managed via [GitHub releases](https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases) by following a thoroughly documented and standardized “release process”.
...
We use the JGI JAMO system for data lifecycle management to store copies of our data on tape on the [NERSC HPSS](https://docs.nersc.gov/filesystems/archive/) system. This allows for efficiently storing related workflow products on tape, and gives us an offline canonical copy of the data.
...
Database backups are performed nightly at NERSC using Spin Cron jobs

## Code quality and maintainability

We _enforce_ the use of a (highly configurable) Python code formatter called [black](https://github.com/psf/black) across most of our repositories.

Another powerful tool we use to check PEP8 compliance is [flake8](https://flake8.pycqa.org/en/latest/).
...
A few tools that we use for type checking are – [mypy](https://github.com/python/mypy) (for static type checking), [pydantic](https://github.com/pydantic/pydantic) (data validation using Python type hints), and [SQLAlchemy](https://github.com/sqlalchemy/sqlalchemy) (Object Relational Mapper).
...
- Approximately a week before the target release date, we ensure that a new version of nmdc-schema has been released and that both nmdc-runtime and nmdc-server depend on the latest version. During this week we test the applications in the dev environment and are more selective about which changes are merged into the `main` branches.
